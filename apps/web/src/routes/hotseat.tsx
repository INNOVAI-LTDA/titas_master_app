...
function HotseatPage() {
  ...
  const criar = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("hotseats")
      .insert({ grupo_id: grupo.id, data_hotseat: data, lider_id: liderId });
  ...
  return (
    <div className="px-8 py-8 max-w-5xl">
      <PageHeader
        title="Hotseat"
        subtitle="Encontros do grupo · cada hotseat pode ter vários gargalos"
      />
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        {hotseats.length === 0 && (
          <p className="text-sm text-muted-foreground">Nenhum hotseat ainda.</p>
        )}
      </div>
    </div>
  );
