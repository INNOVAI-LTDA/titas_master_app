...
function HistoricoPage() {
  ...
  useEffect(() => {
    if (!mes && comps[0]) setMes(comps[0].mes_referencia);
  }, [comps, mes]);
  ...
  const ranking = useMemo(() => {
    return [...fats]
      .map((f) => ({
        membro: membros.find((m) => m.id === f.membro_id)?.nome ?? "—",
  ...
  const exportar = () =>
    compAtual && exportarRelatorioXlsx({ grupo, mesRef: mes, membros, faturamentos: fats });
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      <PageHeader
        ...
        actions={
          <Button variant="outline" onClick={exportar} disabled={!mes}>
        ...
        <Card className="card-innovai p-4">
          ...
          <ol className="space-y-2">
            {ranking.map((r, i) => (
              ...
              >
                <span>
                  <span className="text-gold font-display mr-2">{i + 1}º</span>
                  {r.membro}
                </span>
            ...
            {ranking.length === 0 && (
              <li className="text-sm text-muted-foreground">Sem lançamentos</li>
            )}
          </ol>
        </Card>
      </div>
