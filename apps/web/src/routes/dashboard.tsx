...
function DashboardPage() {
  ...
  const porMes = useMemo(() => {
    ...
    fats.forEach((f) =>
      map.set(f.mes_referencia, (map.get(f.mes_referencia) || 0) + Number(f.valor_bruto)),
    );
  ...
  useEffect(() => {
    if (!mesRanking && comps[0]) setMesRanking(comps[0].mes_referencia);
  }, [comps, mesRanking]);

  const ranking = useMemo(() => {
    return fats
      .filter((f) => f.mes_referencia === mesRanking)
      .map((f) => ({
        nome: membros.find((m) => m.id === f.membro_id)?.nome ?? "—",
        valor: Number(f.valor_bruto),
      }))
    ...
    <div className="px-8 py-8 max-w-7xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        <Card className="card-innovai p-5">
          ...
          <div className="font-display text-2xl text-gold mt-2">
            {ultimo ? formatarMesRefLongo(ultimo.mes) : "—"}
          </div>
        </Card>
        <Card className="card-innovai p-5">
          ...
          <div className="font-display text-2xl text-gold mt-2">
            {ultimo ? formatarBRL(ultimo.total) : formatarBRL(0)}
          </div>
        </Card>
        <Card className="card-innovai p-5">
          <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <div className="font-display text-2xl mt-2 flex items-center gap-2">
            {variacao == null ? (
              ...
              <>
                ...
                <span className={variacao >= 0 ? "text-gold" : "text-destructive"}>
                  {variacao.toFixed(1)}%
                </span>
              </>
            )}
      ...
      <Card className="card-innovai p-5">
            ...
            <SelectContent>
              {comps.map((c) => (
                <SelectItem key={c.id} value={c.mes_referencia}>
                  {formatarMesRefLongo(c.mes_referencia)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <ol className="space-y-2">
          ...
          {ranking.length === 0 && (
            <li className="text-sm text-muted-foreground">Sem dados para este mês</li>
          )}
        </ol>
      </Card>
    </div>
