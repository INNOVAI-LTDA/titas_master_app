...
function GargalosPage() {
  ...
  const { data: hotseats = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .order("data_hotseat", { ascending: false });
  ...
  return (
    <div className="px-8 py-8 max-w-5xl">
      <PageHeader
        title="Gargalos"
        subtitle="Selecione um hotseat para visualizar e registrar gargalos"
      />

      <div className="grid sm:grid-cols-2 gap-3">
        {hotseats.map((h: Hotseat) => {
          const qtd = todos.filter((g: Gargalo) => g.hotseat_id === h.id).length;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-display text-foreground">
                      {new Date(h.data_hotseat).toLocaleDateString("pt-BR")}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Líder: {lider?.nome ?? "—"} · {qtd} gargalo(s)
                    </div>
                  </div>
                </div>
        ...
        {hotseats.length === 0 && (
          <p className="text-sm text-muted-foreground">Nenhum hotseat cadastrado ainda.</p>
        )}
      </div>
    </div>
  );
