...
const cards = [
  ...
  { to: "/membros", title: "Membros", desc: "Cadastro de guerreiros", icon: UserCog },
  {
    to: "/faturamento",
    title: "Faturamento",
    desc: "Lançar faturamento do mês aberto",
    icon: Wallet,
  },
        ...
        <div className="relative px-8 py-20 md:py-28 max-w-5xl">
          <div className="flex items-center gap-3 mb-5">
            <img
              src={logoImg}
              alt="Innovai"
              className="h-14 w-14 rounded-xl border border-gold/30 bg-background/70 object-contain p-1.5 shadow-lg"
            />
            <div>
          ...
          <h1 className="font-display text-5xl md:text-7xl text-gradient-gold leading-none">
            SPARTANOS
          </h1>
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-3xl">
            <ResumoBox
              ...
              value={
                ultimaComp
                  ? formatarMesRefLongo(ultimaComp.mes_referencia)
                  : formatarMesRefLongo(alvo)
              }
              extra={
                ultimaComp && (
                  <Badge
